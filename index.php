...
<div class="grid_1">
      <div class="container">
        ...
        <ul id="flexiselDemo3">
        <?php
        	$sql="SELECT * FROM customer WHERE sex = 'Female'";
...
<div class="grid_1">
      <div class="container">
        ...
        <ul id="flexiselDemo4">
        <?php
        	$sql="SELECT * FROM customer WHERE sex = 'Male'";
        	$result=mysqlexec($sql);
        	if($result){
        		while($row=mysqli_fetch_assoc($result)){
        			$name=$row['firstname'] . " " . $row['lastname'];
					...
					echo "<div class=\"center-middle\">About {$name}</div>";
					echo "</div>";
					echo "<h5><span class=\"m_3\">Profile ID : {$profileid}</span><br>{$age}, {$place}<br>{$job}</h5></a></div>";
					echo "</li>";
...
<div class="grid_2">
    ...
    <div class="footer">
    	<div class="container">
    		...
    		<div class="clearfix"> </div>
    		<div class="copy">
		      <p>Copyright © 2015 Marital . All Rights Reserved  | Design by <a href=>TMS,TT,JK,DK
</a> </p>
	        </div>
    	</div>
    </div>
